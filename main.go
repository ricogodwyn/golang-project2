...
type Movie struct{ //cuman template
	ID string `json:"id"`
...
func deleteMovie (w http.ResponseWriter,r *http.Request){
	...
	for index, item := range movies {
		if item.ID == params["id"]{
			movies = append(movies[:index], movies[index+1:]...) //actually its not deleting but join before and after the index
			break
		}
	}
	json.NewEncoder(w).Encode(movies)
...
func updateMovie(w http.ResponseWriter,r *http.Request){ //combine delete and create
	w.Header().Set("Content-Type","application/json") //set header
	params:=mux.Vars(r)
	for index, item := range movies {
		if item.ID == params["id"]{
